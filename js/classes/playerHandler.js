class Player {
    constructor(idleSpritesheet, thrustSpritesheet, frameWidth, frameHeight, idleFrameCount, idleFrameDuration, thrustFrameCount, thrustFrameDuration) {
    ...
    applyGravity() {
        ...
        for (let i = 0; i < foregroundObjects.length; i++) {
            ...
            for (let i = 0; i < foregroundObjects.length; i++) {
                let planet = foregroundObjects[i];
                if (this.x + this.frameWidth * this.scale >= planet.x && this.x <= planet.x + planet.width && this.y + this.frameHeight * this.scale >= planet.y && this.y <= planet.y + planet.height) {
        ...
        if (finalTotalX > 10 || finalTotalX < -10 || finalTotalY > 10 || finalTotalY < -10) {
            gameOver();
        }
    ...
    collidesWith(object) {
        ...
        if (this.x + this.frameWidth * this.scale >= object.x && this.x <= object.x + object.width && this.y + this.frameHeight * this.scale >= object.y && this.y <= object.y + object.height) {
            return true;
        }
    }

    fakeGravity() { // Not in use by default
        this.y += this.gravity;
        // console.log(this.vy);
    }
    ...
    draw(ctx) {
        ...
        let spritesheet = this.isThrusting ? this.thrustSpritesheet : this.idleSpritesheet;
